...
def main():
    ...
    delta_h_values = [0.095, 0.12, 0.15, 0.18]

    results = []

    print("\nCP-safe MPC parameter study")
    print("=" * 80)
    ...
    for confidence in confidence_levels:
        ...
        for delta_h_max in delta_h_values:
            ...
            violations, steps, dh = evaluate_true_step_safety(
                mpc_path,
                delta_h_max,
            )

            final_dist = np.linalg.norm(mpc_path[-1] - goal)
            reached = final_dist < 0.35

            max_dh = np.max(dh) if len(dh) else 0.0
            ...
            print(
                ...
                f"C={C:7.4f} | "
                f"dh_max={delta_h_max:6.3f} | "
                f"steps={steps:3d} | "
                f"final_dist={final_dist:7.3f} | "
                f"reached={str(reached):5s} | "
                f"viol={violations:2d} | "
                f"max_dh={max_dh:7.4f}"
            )

    # ------------------------------------------------------------
    # Convert results to arrays for plotting
    # ------------------------------------------------------------
    final_distance_grid = np.zeros((len(confidence_levels), len(delta_h_values)))
    violation_grid = np.zeros((len(confidence_levels), len(delta_h_values)))
    reached_grid = np.zeros((len(confidence_levels), len(delta_h_values)))
    C_values = np.zeros(len(confidence_levels))

    for i, confidence in enumerate(confidence_levels):
        for j, delta_h_max in enumerate(delta_h_values):
            match = [
                r for r in results
                if abs(r["confidence"] - confidence) < 1e-9
                and abs(r["delta_h_max"] - delta_h_max) < 1e-9
            ][0]

            final_distance_grid[i, j] = match["final_dist"]
            violation_grid[i, j] = match["violations"]
            reached_grid[i, j] = 1.0 if match["reached"] else 0.0
            C_values[i] = match["C"]

    # ------------------------------------------------------------
    # Plot 1: final distance heatmap
    # ------------------------------------------------------------
    ...
    plt.imshow(
        ...
        extent=[
            min(delta_h_values),
            max(delta_h_values),
            min(confidence_levels) * 100,
            max(confidence_levels) * 100,
        ],
    )
    plt.colorbar(label="final distance to goal [m]")
    plt.xlabel("maximum allowed height change Δh_max [m]")
    plt.ylabel("confidence level [%]")
    plt.title("Reachability tradeoff: final distance to goal")
    plt.tight_layout()

    # ------------------------------------------------------------
    # Plot 2: true safety violations heatmap
    # ------------------------------------------------------------
    plt.figure(figsize=(8, 5))
    plt.imshow(
        violation_grid,
    ...
    plt.colorbar(label="number of true safety violations")
    plt.xlabel("maximum allowed height change Δh_max [m]")
    plt.ylabel("confidence level [%]")
    plt.title("Safety tradeoff: true terrain violations")
    plt.tight_layout()

    # ------------------------------------------------------------
    # Plot 3: conformal margin vs confidence
    # ------------------------------------------------------------
    plt.figure(figsize=(7, 4.5))
    plt.plot(np.array(confidence_levels) * 100, C_values, "o-")
    plt.xlabel("confidence level [%]")
    plt.ylabel("conformal margin C [m]")
    plt.title("Higher confidence increases conformal safety margin")
    plt.grid(True)
    plt.tight_layout()
    ...
    for confidence, delta_h_max in representative_cases:
        delta = 1.0 - confidence
        C, _ = conformal_margin(y_cal, y_cal_pred, delta)

        path, _ = plan_mpc_cp_safe_path(
            start=start,
            goal=goal,
            gp=gp,
            C=C,
        ...
        plt.plot(
            ...
            linewidth=1.8,
            markersize=3.5,
            label=f"{confidence*100:.0f}%, Δh={delta_h_max:.3f}",
        )
    ...
    plt.xlabel("x [m]")
    plt.ylabel("y [m]")
    plt.title("Representative CP-safe MPC paths")
    plt.legend()
    plt.tight_layout()
